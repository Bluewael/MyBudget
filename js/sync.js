...
// (single-device IndexedDB). When signed in, the whole database is serialized
// to a `budget.json` file in the user's private appdata folder on Drive and
// re-uploaded on every data change (debounced 3s). Reads happen at sign-in
// time; after that IndexedDB is authoritative in-session.
//
// Design notes:
// - Sync metadata (OAuth client id, last-sync timestamp, tokens) lives in
//   localStorage, NOT in db.settings. This avoids two problems: (1) writing
//   the timestamp would fire data-changed and cause a sync loop, and (2) each
//   device configures its own OAuth so the client id should not sync.
...
const DRIVE_FILE_NAME = 'budget.json';
const SCOPES = [
  'https://www.googleapis.com/auth/drive.appdata',
  'https://www.googleapis.com/auth/userinfo.email'
].join(' ');

const LS_LAST_SYNC = 'budget_sync_last_sync';
...
// click "Sign in with Google" and never see the setup guide.
//
...
// those origins.
//
// When to leave this blank: you want each user to bring their own Client ID
...
async function initialSync() {
  ...
  const choice = confirm(
    `Google Drive already has a budget.json for this account (${relativeAge}).\n\n` +
...
async function isDbNonEmpty() {
  const [c, t] = await Promise.all([db.categories.count(), db.transactions.count()]);
  return c > 0 || t > 0;
}
...
async function downloadAndReplace() {
  ...
  notifyState();
  // Fire data-changed so all pages refresh from the new data
  window.dispatchEvent(new CustomEvent('data-changed'));
}

async function hydrateFromPayload(data) {
  // Same shape as the Backup export/import format (versions 1–3).
  window.sync._hydrating = true;
  try {
    await db.transaction('rw',
      db.categories, db.transactions, db.rules, db.budgets, db.yearlyBudgets, db.settings,
      async () => {
        ...
        if (data.yearlyBudgets?.length) await db.yearlyBudgets.bulkAdd(data.yearlyBudgets);
...
async function uploadNow() {
  ...
  try {
    ...
    const payload = {
      ...
      categories, transactions, rules, budgets, yearlyBudgets, settings
    };
...
// Alpine component that renders the sign-in card on the Backup page. It mirrors
